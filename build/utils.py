...
def get_context() -> Path:
    """Return the container build context.
...
def get_image_reference(
    ...
    :return:
    """
    reference: str = f"{registry}/pfeiffermax/valheim-dedicated-server:{tag}"
    return reference


def get_valheim_build_id() -> str:
    ...
    info: dict = client.get_product_info(apps=[896660], timeout=1)
    build_id: str = info["apps"][896660]["depots"]["branches"]["public"]["buildid"]
    return build_id
...
def tag_exists(build_id: str) -> bool:
    ...
    response = requests.get(
        "https://hub.docker.com/v2/namespaces/pfeiffermax/repositories/valheim-dedicated-server/tags"
    )
